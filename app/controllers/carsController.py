...
async def create_car(db: AsyncSession, car: carsSchema.CarBase):
    db_car = Cars(**car.model_dump(exclude_none=True))
    db.add(db_car)
    await db.commit()
...
async def get_cars(db: AsyncSession, skip: int = 0, limit: int = 10):
    result = await db.execute(select(Cars).offset(skip).limit(limit if limit > 0 else None))
    return result.scalars().unique().all()


async def get_car(db: AsyncSession, car_id: int):
    result = await db.execute(select(Cars).where(Cars.id == car_id))
